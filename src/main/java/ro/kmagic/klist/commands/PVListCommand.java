...
public class PVListCommand extends Command {

    public PVListCommand() {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        for (ServerInfo server : ProxyServer.getInstance().getServers().values()) {
            ...
            int[] playercount = new int[]{Math.max(getMaxPlayers(sender, config, server) - BungeeVanishAPI.getInvisiblePlayers().size(), 0)};

            if(playercount[0] <= 0)
                continue;

            int totalPlayers = ProxyServer.getInstance().getOnlineCount();
            int serverPlayerCount = server.getPlayers().size();

            totalPlayers = Math.max((totalPlayers - BungeeVanishAPI.getInvisiblePlayers().size()), 0);
            serverPlayerCount = Math.max((serverPlayerCount - BungeeVanishAPI.getInvisiblePlayers().size()), 0);

            float percent = totalPlayers == 0 ? 0.0F : (serverPlayerCount * 100.0F / totalPlayers);
            ...
            serverFormat = serverFormat.replace("{PLAYER_AMOUNT}", "" + serverPlayerCount);
    ...
    private String format(List<ServerInfo> servers, HashMap<String, String> formattedRows, Configuration config) {
        ...
        servers.sort(Comparator.comparingInt(a -> a.getPlayers().size() - BungeeVanishAPI.getInvisiblePlayers().size()));
    ...
    private int getMaxPlayers(CommandSender sender, Configuration config, ServerInfo server) {
        ...
        Future<?> future = executor.submit(() -> {
            ...
            if((server.getPlayers().size() - BungeeVanishAPI.getInvisiblePlayers().size()) < 1)
                if(config.getBoolean("hide-empty-servers"))
                    return;
